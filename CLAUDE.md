...
사이트. 정적 React + Babel-standalone 페이지. 콘텐츠는 `content/`의 JSON.
편집은 `/admin/`의 Sveltia CMS 또는 헤더 KIMES 로고 5x 클릭으로 열리는
인플레이스 에디터로 GitHub 커밋.
...
- `#admin` / `#edit` URL 해시 트리거 — 같은 로그인 플로우 진입점
- Sveltia CMS at `/admin/` — GitHub PAT 인증, 콘텐츠 직접 커밋
- content-editor.js의 GitHub Publish 버튼
