...
function DownloadsPage() {
  ...
  return (
    <PageShell
      ...
      title="Downloads"
      subtitle="파일 다운로드"
      lede="KIMES 2027 BI 파일 모음. 통합 패키지 또는 카테고리별로 받아주세요."
    >
      {/* 통합 패키지 ---------------------------------------------------- */}
      ...
      <label className="dl-terms">
        ...
        />
        <span>
          위 파일은 KIMES {window.KIMES_EVENT.event.year} 참가 표기 목적에만 사용하며,
          권한 범위(→ <Link to="/permissions">/permissions</Link>)를 준수합니다.
        </span>
      </label>
      ...
      <p className="dl-changelog">
        파일 변경 이력 → <Link to="/changelog">/changelog</Link>
      </p>
    </PageShell>
  );
}
